...
int main()
{
    ...
    double t1,t2,t3;
    FILE *f1,*f2,*f3,*fsave;
    f1=fopen("RandomCase.txt","a");
    f2=fopen("BestCase.txt","a");
    f3=fopen("WorstCase.txt","a");
    fsave=fopen("Record.txt","a");
    //to save data of time taken
    ...
    fprintf(fsave,"%d %lf %lf %lf\n",x,t1,t2,t3);
    fprintf(f3,"%d %lf\n",x,t3);

    //closing files after finished
    fclose(f1);
    fclose(f2);
    fclose(f3);
    fclose(fsave);
